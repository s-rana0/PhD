...
## 1) A function for cox regression without adjustments (univariate)
cox.reg.uni <- function(followup.df = Long.test.cox.df, event = 'BCR',
                          event.time = 'time.to.event', no_features = 1145,
                          save = c('Y', 'N'), save.path){
  
  feature_names <- colnames(followup.df)[1:no_features]
  
  ## Initialise empty df to save cox regression HR and p-value
  isomiRs.SA.res <- data.frame(matrix(NA, nrow= no_features, ncol = 6))
  colnames(isomiRs.SA.res) <- c('miR','mir.HR','mir.pval','mir.LI','mir.UI','mir.fdr')
  rownames(isomiRs.SA.res) <- isomiRs.SA.res$miR <- feature_names
  
  ## perform cox regression 
  for(i in 1:no_features){
    # Regression
    cox.res <- coxph(Surv(followup.df[, event.time], followup.df[,event])~
                       as.numeric(followup.df[,i]))
    # save results
    isomiRs.SA.res[i,2:5] <- c(summary(cox.res)$coefficients[1,c(2,5)], 
                               summary(cox.res)$conf.int[1,c(3,4)])
    print(paste('cox regression:', i))
  }
  ## Adjust p-values
  isomiRs.SA.res$mir.fdr <- p.adjust(isomiRs.SA.res$mir.pval, method='BH')
  
  # Save
  if (save == 'Y'){
    write.csv(isomiRs.SA.res, save.path, row.names=T) 
  }
  
  print('Finished running')
  return(isomiRs.SA.res)
}

## 2) A function for cox regression for individual clinical variables (univariate)
cox.reg.adjust <- function(followup.df = Long.train.cox.df, event = 'BCR',
                           event.time = 'time.to.event', control_age = 'age',
                           control_pT='tumor.stage', control_psa = 'psa',
                           control_gleason='gleason.score'){
  
  ## Convert adjust variables to appropriate categories
  # Age + PSA
  followup.df[,control_age] <- as.numeric(followup.df[,control_age])
  followup.df[,control_psa] <- as.numeric(followup.df[,control_psa])
  # gleason
  followup.df[,control_gleason] <- ifelse(followup.df[,control_gleason] >= 8, '>=G8', 'G6-G7')
  followup.df[,control_gleason] <- factor(followup.df[,control_gleason], levels=c('G6-G7', '>=G8'))
  # pT stage
  followup.df[,control_pT] <- toupper(gsub('[a-z]$', '', tolower(followup.df[,control_pT])))
  followup.df[,control_pT] <- ifelse(followup.df[,control_pT]=='T1' | followup.df[,control_pT]=='T2', 'T1-T2', 'T3-T4')
  followup.df[,control_pT] <- factor(followup.df[,control_pT], levels=c('T1-T2', 'T3-T4'))
  
  ## Initialise empty df to save cox regression HR and p-value
  isomiRs.SA.res <- data.frame(matrix(NA, nrow= 4, ncol = 5))
  colnames(isomiRs.SA.res) <- c('HR','pval','LI','UI','variables')
  rownames(isomiRs.SA.res) <- c('age','G6-G7Vs>=G8','T1+T2VsT3+T4','psa')
  isomiRs.SA.res$variables <- c('age','G6-G7Vs>=G8','T1+T2VsT3+T4','psa')
  
  ## perform cox regression
  # AGE
  cox.res.age <- coxph(Surv(followup.df[, event.time], followup.df[,event])~
                         followup.df[,control_age])
  # Gleason
  cox.res.gleason <- coxph(Surv(followup.df[, event.time], followup.df[,event])~
                             followup.df[,control_gleason])
  # pT stage
  cox.res.pT <- coxph(Surv(followup.df[, event.time], followup.df[,event])~
                        followup.df[,control_pT])
  # psa
  cox.res.psa <- coxph(Surv(followup.df[, event.time], followup.df[,event])~
                         followup.df[,control_psa])
  
  # save results
  # Append age
  isomiRs.SA.res['age',1:4] <- c(summary(cox.res.age)$coefficients[1,c(2,5)], summary(cox.res.age)$conf.int[1,c(3,4)])
  # Append gleason
  isomiRs.SA.res['G6-G7Vs>=G8',1:4] <- c(summary(cox.res.gleason)$coefficients[1,c(2,5)], summary(cox.res.gleason)$conf.int[1,c(3,4)])
  # Append pTumour stage
  isomiRs.SA.res['T1+T2VsT3+T4',1:4] <- c(summary(cox.res.pT)$coefficients[1,c(2,5)], summary(cox.res.pT)$conf.int[1,c(3,4)])
  # Append psa
  isomiRs.SA.res['psa',1:4] <- c(summary(cox.res.psa)$coefficients[1,c(2,5)], summary(cox.res.psa)$conf.int[1,c(3,4)])
...
## 3) Suer dataset only. A function for cox regression for individual clinical variables (univariate)
cox.reg.adjust.Suer <- function(followup.df = Long.train.cox.df, event = 'BCR',
                           event.time = 'time.to.event', control_age = 'age',
                           control_psa = 'psa', control_gleason='gleason.score'){
  ...
  ## Initialise empty df to save cox regression HR and p-value
  isomiRs.SA.res <- data.frame(matrix(NA, nrow= 3, ncol = 5))
  colnames(isomiRs.SA.res) <- c('HR','pval','LI','UI','variables')
  rownames(isomiRs.SA.res) <- c('age','G6-G7Vs>=G8','psa')
  isomiRs.SA.res$variables <- c('age','G6-G7Vs>=G8','psa')
  ...
  print('Finished running :3')
  return(isomiRs.SA.res)
}


## 4) A function for cox regression with adjustments (multivariate)
cox.reg.multi <- function(followup.df = Taylor.cox.df, event = 'BCR',
                          control_pT='PathStage', event.time = 'event_time',
                          no_features = 373, control_gleason = 'PathGGS',
                          control_psa='PreDxBxPSA', save = c('Y', 'N'),  save.path){
  
  feature_names <- colnames(followup.df)[1:no_features]
  
  ## Convert adjust variables to appropriate variables
  # PSA
  followup.df[,control_psa] <- as.numeric(followup.df[,control_psa])
  # Gleason
  followup.df[,control_gleason] <- ifelse(followup.df[,control_gleason] >= 8, '>=G8', 'G6-G7')
  ...
  for(i in 1:no_features){
    # Regression
    cox.res <- coxph(Surv(followup.df[, event.time], followup.df[,event])~
                       as.numeric(followup.df[,i]) + followup.df[,control_pT] + 
                       followup.df[,control_gleason] + followup.df[,control_psa])
    # save results
    # Append miR first
    isomiRs.SA.res[i,2:5] <- c(summary(cox.res)$coefficients[1,c(2,5)], 
                               summary(cox.res)$conf.int[1,c(3,4)])
  ...
  # Save
  if (save == 'Y'){
    write.csv(isomiRs.SA.res,  save.path)
  }
...
## 5) Meta analysis for univariate
ME_miR_uni <- function(data=SA.cox.uni, miR='miR-423-3p', miR.lab='', test='REML', variable='miR'){
  
  # Gather info for that miR from all the datasets into one df 
  temp <- lapply(data, function(x) 
    as.data.frame(x[grep(miR, x[,variable]), c('mir.HR', 'mir.pval', 'mir.LI', 'mir.UI', 'mir.fdr','author')]))
  SA.cox.uni.df <- do.call('rbind', temp)
  
  # Calculate log effect size + standard errors
  SA.cox.uni.df <- SA.cox.uni.df %>% 
    mutate(log.HR = log(mir.HR)) %>% 
    mutate(SEI = (log(mir.UI) - log(mir.LI))/(2*1.96))
  MA.data <- data.frame(trial=seq(1, nrow(SA.cox.uni.df)),
                        author=SA.cox.uni.df$author,
                        SA.cox.uni.df)
...
############################################################
############ META-ANALYSIS: Clinical variables #############
############################################################
...
## 4) Run RMA + save plots
pdf('results/figures/age_MA_200605.pdf', height=4, width=6.5)
par(mar=c(4.5,4,0,2))
...
# Header
text(-0.44,7.5, 'Author (Study ID)', pos=4, cex=1.1, font=2)
text(0.35,7.5, 'Hazard Ratio [95% CI]', pos=2, cex=1.1, font=2)
dev.off()

pdf('results/figures/gleason_MA_200605.pdf', height=4, width=6.5)
par(mar=c(4.5,4,0,2))
...
text(8,7.5, 'Hazard Ratio [95% CI]', pos=2, cex=1.1, font=2)
dev.off()

pdf('results/figures/pT_MA_200605.pdf', height=4, width=6.5)
par(mar=c(4.5,4,0,2))
...
# Header
text(-8.5,6.5, 'Author (Study ID)', pos=4, cex=1.1, font=2)
text(8,6.5, 'Hazard Ratio [95% CI]', pos=2, cex=1.1, font=2)
dev.off()

pdf('results/figures/psa_MA_200605.pdf', height=4, width=6.5)
par(mar=c(4.5,4,0,2))
...
# Header
text(-0.73,7.5, 'Author (Study ID)', pos=4, cex=1.1, font=2)
text(0.55,7.5, 'Hazard Ratio [95% CI]', pos=2, cex=1.1, font=2)
dev.off()



##########################################################
########### META-ANALYSIS: Univariate analysis ###########
##########################################################
...
temp <- res.MA.uni.df[res.MA.uni.df$p.val < 0.05,]
temp$miR[temp$HR < 1]; temp$miR[temp$HR > 1]
table(temp$Q_p < 0.05); range(temp$I_sq)

## 5) Plot the figures for the significant ME res
pdf('results/figures/REM_uni_200605.pdf', height=4, width=6.5)
par(mar=c(4.5,4,0,2))
res.MA.uni <- lapply(sig.ME.uni, function(x) ME_miR_uni(data=SA.cox.uni, miR=x, miR.lab=paste0('\n \n \n',x), test='REML', variable='miR'))
dev.off()
...
temp <- res.MA.multi.df[res.MA.multi.df$p.val < 0.05,]
temp$miR[temp$HR < 1]; temp$miR[temp$HR > 1]; paste(temp$miR[temp$HR > 1], collapse=', ')
range(temp$I_sq); table(temp$p.val < 0.1)
# Overlap between uni + multi-variate
intersect(common.miRs, common.miRs.multi) #162
intersect(sig.ME.uni, sig.ME.multi) #10


## 5) Plot the figures for the significant ME res
pdf('results/figures/REM_multi_200605.pdf', height=4, width=6.5)
par(mar=c(4.5,4,0,2))
res.MA <- lapply(sig.ME.multi, function(x) ME_miR_multi(data = SA.cox.multi, variable.name=x, miR.lab=paste0('\n \n \n',x), test='REML', variable.interest='mir'))
dev.off()



###############################################################
####### Statistics on clinical variable for the studies #######
###############################################################
...
### Save the images 
pdf('results/figures/age_200605.pdf', height=4, width=5)
box.age; dev.off()
pdf('results/figures/PSA_200605.pdf', height=4, width=5)
box.psa; dev.off()
pdf('results/figures/gleason_200605.pdf', height=4, width=5)
bar.plot.gleason.prop; dev.off()
pdf('results/figures/stage_200605.pdf', height=4, width=5)
bar.plot.pT.prop; dev.off()
pdf('results/figures/BCR_200605.pdf', height=4, width=5)
plot.BCR; dev.off()


### Calculating proportions
prop.table(data.matrix(ME_stats[,c("samples.BCR", "samples.noBCR")]), 1)*100 # prop BCR
prop.table(data.matrix(ME_stats[c(1:4,6),c("stage.T1","stage.T2","stage.T3","stage.T4")]), 2)*100 # prop pT
prop.table(data.matrix(ME_stats[,c("ggs.6","ggs.7","ggs.8_10")]), 2)*100 # prop GG
sapply(c('TCGA-PRAD','GSE88958','GSE46738','GSE26247','GSE26245','GSE21036'), function(x) median(hist.age$age[hist.age$study==x], na.rm=T)) #age
sapply(c('TCGA-PRAD','GSE88958','GSE46738','GSE26247','GSE26245','GSE21036'), function(x) median(hist.psa$psa[hist.psa$study==x], na.rm=T))# psa
...
par(mar=c(4.5,4,0,2))
# Uni
#del.res <- lapply(ind, function(x) ME_miR_uni(data=SA.cox.uni, miR=x, miR.lab=paste0('\n \n \n',x), test='REML', variable='miR.V22'))
# multi
del.res <- lapply(ind, function(x) ME_miR_multi(data = SA.cox.multi, variable.name=x, miR.lab=paste0('\n \n \n',x), test='REML', variable.interest='mir'))
dev.off()


### miR signature
ind <- grep('miR-17|miR-18a|miR-19a|miR-20a|miR-19b-1|and|miR-92a-1', common.miRs, ignore.case = T, value = T)
intersect(ind, sig.ME.multi); intersect(ind, sig.ME.uni)

pdf('results/figures/REM_multi_200605_SA_mult.pdf', height=4, width=6.5)
par(mar=c(4.5,4,0,2))
# Uni
#del.res <- lapply(ind, function(x) ME_miR_uni(data=SA.cox.uni, miR=x, miR.lab=paste0('\n \n \n',x), test='REML', variable='miR.V22'))
# multi
